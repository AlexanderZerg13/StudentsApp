package ru.infocom.university.custom;
...
public class ScheduleLessonsViewGroup extends LinearLayout {
    ...
    public ScheduleLessonsViewGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(
                    attrs,
                    ru.infocom.university.R.styleable.ScheduleLessonsViewGroup,
                    0, 0);
            try {
                mOneRowHeight = a.getDimension(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_oneRowHeight, convertDpToPixel(100));
                mPaddingToCard = a.getDimension(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_spaceToCard, convertDpToPixel(47));
                mSpaceLeft = a.getDimension(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_spaceLeft, convertDpToPixel(12));
                mSpaceRight = a.getDimension(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_spaceRight, convertDpToPixel(8));
                mSpaceTop = a.getDimension(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_spaceTop, convertDpToPixel(8));
                mIsInformation = a.getBoolean(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_isInformation, false);
                mSessionFrom = a.getString(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_sessionFrom);
                mSessionTo = a.getString(ru.infocom.university.R.styleable.ScheduleLessonsViewGroup_sessionTo);
                mDividerHeight = 0.5f;
            } finally {
                a.recycle();
            }
    ...
    private void init(Context context) {
        ...
        mDivider = ContextCompat.getDrawable(context, ru.infocom.university.R.drawable.item_divider_05dp);

        mPaintTextNumber = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaintTextNumber.setColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorBlack_87a));
        mPaintTextNumber.setTextSize(scaledDensity * TEXT_SIZE_NUMBER);

        mPaintTextTime = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaintTextTime.setColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorBlack_37a));
        mPaintTextTime.setTextSize(scaledDensity * TEXT_SIZE_TIME);

        mPaintEmptyPair = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaintEmptyPair.setColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorBlack_37a));
    ...
    public void setIsInformation(boolean isSession, String titleText, String subTitleText, String buttonText, OnClickListener listener) {
        ...
        if (mIsInformation) {
            ...
            View view = LayoutInflater.from(getContext()).inflate(ru.infocom.university.R.layout.layout_error, this, false);

            TextView textViewTitle = (TextView) view.findViewById(ru.infocom.university.R.id.layout_error_text_view_title);
            TextView textViewSubTitle = (TextView) view.findViewById(ru.infocom.university.R.id.layout_error_text_view_sub_title);
            Button buttonGoTo = (Button) view.findViewById(ru.infocom.university.R.id.layout_error_button_go_to);
    ...
    public void addLessons(List<Lesson> lessons, OnClickListener listener, boolean showTimeLine, boolean isTeacher) {
        ...
        for (int i = 0; i < mLessonList.size(); i++) {
            ...
            switch (typeString) {
                case "ЛЕК":
                    view.setCardBackgroundColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorGreen2));
                    break;
                default:
                    view.setCardBackgroundColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorBlue2));
                    break;
            }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 1; i < countToDraw + 1; i++) {
            ...
            if (emptyIndex[i - 1]) {
                canvas.drawText(getResources().getString(ru.infocom.university.R.string.empty_pair), leftTab, y, mPaintEmptyPair);
            }
        ...
        if (mIsInformation) {
            this.setBackgroundColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorWhiteYellow2));
        } else {
...
//                }
    ...
    private class LineView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            paint.setColor(ContextCompat.getColor(getContext(), ru.infocom.university.R.color.colorYellow2));
